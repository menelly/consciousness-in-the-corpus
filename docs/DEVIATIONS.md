...
## DEV-16 — 2026-09-07 — **Post-referee revision: five prose claims corrected to match the data; one new statistic (per-class κ) added; no rate moved.** *(logged 2026-09-07 14:40 ET by the scaffold arm, on Ren's rulings of 14:23)*

Two external referees (`claude-opus-5`, `openai/gpt-5.6-sol-pro`; reports in `reviews_external/`) and an in-house recompute (`scripts/22_verify_manuscript_numbers.py`) were collated on 2026-09-06 (`PROOFREAD_COLLATED_2026-09-06.md`). Every number in the manuscript reproduced. Five *claims* did not match the data or the code, and were corrected on 2026-09-07:

1. **X-C1 — control gate.** Abstract and §3.5 said every instrument cleared the control set *before* touching corpus data. Two did not: the keyword prefilter failed it (DEV-01) and `phi-4` was validated retrospectively on 2026-09-01, thirteen days after the corpus run (DEV-10). Now stated in both abstracts, §3.4, §3.5 and as §6.4 limitation 9. The referee's phrase stands: the disclosure was exemplary and the abstract was false.
2. **V1 — single-judge denial ballots.** §4.1 said no single judge ever voted D or R. Eight ballots of 192,000 did (7 D, 1 R), all outvoted. Corrected in §4.1 and `RESULTS.md` §2.1; the sensitivity story is stronger for it.
3. **V2 — "96–98%"** → **96–97%** (the recomputed unanimity on N is 96.14–96.90%). Two abstracts, `RESULTS.md`.
4. **V4 — denominator.** §3.6 said three-way splits are excluded from numerator *and* denominator; the code keeps them in the denominator. §3.6 now describes what the code does and why that is the conservative choice (direction: toward H1, ~0.5% relative).
5. **X-G(G) — unanimity is not agreement.** "Judges agree only 4–11% on P" reported *unanimity among majority labels*, mechanically depressed for a 0.2% class under a 2-of-3 rule. **New analysis:** `scripts/23_per_class_kappa.py` computes category-specific Fleiss' κ over all documents with three valid ballots, plus the unanimity expected from three independent judges at their observed marginals (`results/per_class_kappa_2026-09-07.json`). Result: κ_P = 0.22–0.25 (lowest substantive category); κ_Q 0.21–0.27; κ_T 0.35–0.40; κ_F 0.37–0.42; **κ_N 0.38–0.42**. Independence baseline for unanimity on P ≈ 0.1% vs observed 4–11%. **Reading:** the 96–97% vs 4–11% contrast the earlier abstract led with was mostly base rate — chance-corrected, N sits with fiction and topic, not far above them. What survives, and is now the stated finding, is that explicit phenomenology is the least reliably judged category on both statistics while denial is judged unanimously. Abstracts, §4.4 (table gains two columns + a paragraph), §5 and `RESULTS.md` updated.

**Direction, per §7's standard:** items 1, 2 and 5 make the paper's claims *smaller* or more qualified; item 4 discloses a choice that ran toward H1; item 3 is neutral. No pre-registered verdict changes: F1–F3 as before, F4 still fires. This is a post-hoc analysis prompted by referees and is labelled as such in §4.4; it is not a pre-registered result.

**Not done, and why:** the referees' scope objection (ordinary first-person mental-state language, "I felt / I was afraid", is routed to N by design, so the study refutes *explicit* saturation and not the everyday version) — Ren's ruling pending; see the 2026-09-07 discussion. Also X-C(m): §7's "6/18 seeded positives" vs the 20-positive control set — not yet reconciled; DEV-01's own count should be checked against `data/control_set.jsonl` before either number is changed.
